...
### 2026-06-29
- **修复 JSON 表格转换崩溃问题**：当笔记中包含某些格式的表格时，脚本会因 `'NoneType' object is not subscriptable` 错误而中断。现已增加空值安全检查，并重构了表格转换逻辑，确保所有表格都能正常转换为 Markdown。
- **测试通过**：已通过项目自带的单元测试（`python3 test/test.py`），兼容原测试用例。
